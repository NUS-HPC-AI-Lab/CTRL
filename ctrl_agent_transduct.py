...
class CTRL:
    ...
    def test_with_val(self, verbose=True):
        ...
        if self.args.dataset in ['ogbn-arxiv','ogbn-arxiv-xrt']:
            model = GCN(nfeat=feat_syn.shape[1], nhid=self.args.hidden, dropout=0.5,
    ...
    def train(self, verbose=True):
        ...
        if args.init_way == 'Center':
            ...
            for c in range(data.nclass):
                ...
                labels_init = kmeans_init.fit_predict(features_c)
                # random_sample_indices = [random.choice(np.where(labels_init == i)[0]) for i in range(n_clu)]
                selected_indices = []
        ...
        for it in range(args.epochs+1):
            if args.dataset in ['ogbn-arxiv','ogbn-arxiv-xrt']:
            ...
            if args.dataset == 'ogbn-arxiv':
                eval_epochs = [20,30,50,100,150,200,250,300,400,500, 600,800, 1000, 1200, 1600, 2000, 3000, 4000, 5000]
            elif args.dataset == 'ogbn-arxiv-xrt':
                eval_epochs = [200,250,300,400,500, 600,800, 1000, 1200, 1400,1600,1800, 2000, 3000, 4000, 5000]
            ...
            # if verbose and (it+1) % 50 == 0:
                res = []
                runs = 1 if args.dataset in ['ogbn-arxiv','ogbn-arxiv-xrt'] else 3
                for i in range(runs):
                    if args.dataset in ['ogbn-arxiv','ogbn-arxiv-xrt']:
                ...
                res = np.array(res)
                try:
                    train_acc_values.append(res.mean(0)[0]) 
                    val_acc_values.append(res.mean(0)[1])  
                except:
                    pass
