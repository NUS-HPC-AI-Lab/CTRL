...
class SGC(nn.Module):
    ...
    def forward_sampler(self, x, adjs):
        ...
        if self.multi_label:
            ...
            return F.log_softmax(x, dim=1)
#     def forward_sampler(self, x, nids_list,adjs_list):

#         # 对输入的nids进行拼接
#         nids = torch.cat(nids_list, dim=0)
#         x = x[nids]
#         print(x.shape)
...
#         # # 将所有类的 adjs 进行拼接
#         # print(len(adjs_list[0]))
#         # print(adjs_list[0][0])

#         # concatenated_adjs = [torch_sparse.cat([adj for adj, _, _ in adjs], dim=0) for adjs in adjs_list]
#         concatenated_adjs = []
#         # print(adjs_list)
#         # for i in range(2):  # Since there are two sparse tensors in each element
 
#         #     adjs = [adjs_list[j][i].adj_t for j in range(len(adjs_list))]  # Extract the ith sparse tensor from each element
#         #     concatenated_adj = torch_sparse.cat(adjs, dim=1)
#         #     # concatenated_adj = torch_sparse.cat(adjs, dim=1)
#         #     concatenated_adjs.append(concatenated_adj)
# # [n_id]
#         adj_t_combined = adjs_list[0].adj_t.cat(adjs_list[1].adj_t, adjs_list[2].adj_t, adjs_list[3].adj_t, dim=0)

#         print(len(concatenated_adjs))
#         print(concatenated_adjs)
#         # 进行模型的前向传播
#         for ix, adj in enumerate(concatenated_adjs):
#             x = torch_sparse.matmul(adj.to(self.device), x)
