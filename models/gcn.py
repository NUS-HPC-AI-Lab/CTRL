...
class GCN(nn.Module):
    ...
    def fit(self, feat, adj, labels, idx_train, data, train_iters=600, initialize=True, verbose=False):
        # print(data)
        if initialize:
            self.initialize()
        ...
        for i in range(train_iters):
            ...
            with torch.no_grad():
                ...
                s_val = F.nll_loss(output[data.idx_val], labels_val)
    ...
    def _train_with_val(self, labels, data, train_iters, verbose, adj_val=False):
        ...
        for i in range(train_iters):
            ...
            loss_train.backward()
                        ...
                        # 获取梯度
            # grads = [param.grad for param in list(self.parameters())] 
            # # 转换为detach及clone
